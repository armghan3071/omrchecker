...
function workerServicePlugin(settings = defaultSettings) {
      ...
      try {
        ...
        if (imports.length > 0) {
          for (const imp of imports) {
            if (imp.file && new RegExp("\\." + settings.ext + `(\\.ts|\\.js|)["']$`).test(imp.file)) {
              usedImports.push({
              ...
              res = res.replace(
                imp.file,
                `"change-to-worker${imp.file.replace(/"/g, "").replace(/;/g, "")}";`
              );
            }
          }
        }
...
function getImportsfromAST(ast) {
  const body = ast.body;
  const imports = body.filter(
    (node) => node.type === "ImportDeclaration"
  );
  const res = imports.map((entry) => {
    return {
...
export {
  workerServicePlugin as default
};
